...
local function TriggerCruiseControl()
    if CruisedSpeed == 0 and IsDriving() then
        if GetVehicleSpeed() > 0 and GetVehicleCurrentGear(GetVehicle()) > 0 then
            ...
            TriggerEvent('ug-hud:Seatbelt:Client:ToggleCruise')
            UgCore.Functions.Notify('Cruise Control', 'Cruise Control Activated at '.. CruisedSpeedMph ..' MP/H!', 'info', 3000)
            CreateThread(function()
                while CruisedSpeed > 0 and IsInVehicle() == Player do
                    ...
                    if not IsTurningOrHandBraking() and GetVehicleSpeed() <
                        ...
                        CruisedSpeed = 0
                        TriggerEvent('ug-hud:Seatbelt:Client:ToggleCruise')
                        UgCore.Functions.Notify('Cruise Control', 'Cruise Control Deactivated!', 'error', 3000)
                        Wait(2000)
                        break
                    end
...
RegisterCommand('togglecruise', function()
    ...
    if IsDriver() then
        if vehicleClasses[vehClass] then
            ...
            UgCore.Functions.Notify('Cruise Control', 'Cruise Control Unavaiable!', 'error', 3000)
        end
    end
end, false)
