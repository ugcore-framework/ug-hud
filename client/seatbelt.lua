...
RegisterNetEvent('ug-hud:Seatbelt:Client:UseHarness', function(ItemData) -- On Item Use (registered server side)
    ...
    if inveh and class ~= 8 and class ~= 13 and class ~= 14 then
        if not harnessOn then
            ...
            UgCore.Functions.CreateProgressBar('harness_equip', 'Attaching Race Harness', 5000, false, true, {
            ...
            LocalPlayer.state:set('inv_busy', true, true)
            UgCore.Functions.CreateProgressBar('harness_equip', 'Removing Race Harness', 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        UgCore.Functions.Notify('Seatbelt', 'You must be in a car to use this!', 'error', '5000')
    end
end)
...
function ToggleSeatbelt()
    if seatbeltOn then
        seatbeltOn = false
        TriggerEvent('ug-hud:Seatbelt:Client:ToggleSeatbelt')
